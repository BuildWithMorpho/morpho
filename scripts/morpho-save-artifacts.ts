...
const morpho_build_artifacts = packages_core + '/morpho-build';
const buildFiles = ['CHANGELOG.md', 'package.json', 'dist'];

(async () => {
  await $('rm', '-rf', morpho_build_artifacts);
  ...
  process.chdir(`${morpho_build_artifacts}`);
  try {
    await $('git', 'checkout', branch);
  } catch (e) {
    await $('git', 'checkout', '-b', branch);
  }
  await $('rm', '-rf', ...(await expand(morpho_build_artifacts)));
  process.chdir(`${packages_core}`);
  await $('cp', '-r', ...buildFiles, morpho_build_artifacts);
  process.chdir(`${morpho_build_artifacts}`);
  await $('git', 'add', '--all');
  await $(
    'git',
  ...
  console.log('##############################################################');
  console.log('##############################################################');
  console.log(`### https://github.com/BuilderIO/morpho-build/commit/${dstSHA}`);
  console.log('##############################################################');
  console.log('##############################################################');
