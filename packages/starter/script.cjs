...
const main = async () => {
  p.intro(`Welcome to Morpho! Let's get started.`);

  projectName = await p.text({
    message: 'What is your Morpho project name?',
   ...
   */
  const updateMorphoConfig = (filePath) => {
...
module.exports = ${JSON.stringify(config, null, 2)}`;
  ...
  const copy = (src, dest) => {
    ...
    fs.readdirSync(src).forEach((entry) => {
      ...
      // update morpho.config.cjs
      if (entry === 'morpho.config.cjs') {
        try {
          const newConfig = updateMorphoConfig(srcPath);

          fs.writeFileSync(destPath, newConfig);
        } catch (error) {
          p.note('Error processing `morpho.config.cjs`. Copying as-is.');
          console.error(error);
