      ...
      Property(node) {
        if (
          ...
          if (stateKeys.includes(lowercasedStateName)) {
            context.report({
              node,
              message: `Cannot name a state property \`${propName}\` because of a collision with Morpho-generated code for the state property \`${lowercasedStateName}\`. Please use a different name.`,
            });
          }
        }
      },
