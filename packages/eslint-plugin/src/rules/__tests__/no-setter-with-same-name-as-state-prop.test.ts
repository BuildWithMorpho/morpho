...
ruleTester.run('no-setter-with-same-name-as-state-prop', rule, {
  valid: [
    ...
    // Doesn't apply to none morpho files
    {
      ...opts,
      code: `
      import { useStore } from "@builder.io/morpho";
...
export default function MyComponent(props) {
  ...
  return (
    <div>
      ...
      `,
      errors: [
        'Cannot name a state property `setFoo` because of a collision with Morpho-generated code for the state property `foo`. Please use a different name.',
      ],
    },
  ],
});
