...
- cb7be32: [all] export `types.ts` to enable `@type` comment for JS like this: `/** @type {import('@builder.io/morpho').ToReactOptions} */`
...
- 559cf86: Don't output JSX attributes for Morpho generator that are invalid (e.g. start with @ or :)
...
- 3d44b65: Properly encode "<", ">", and other chars in text in Morpho JSX output
...
- 8e9e3c3: [All] Normalize name of morpho node before assigning it to className
...
- 10a168d: [Builder] preserve bound media query styles when converting to Morpho
...
- 6f6db62: [React] Refactor how `react` handles morpho `Fragment`.
...
- 499b4b7: [Morpho] feat: add returnArray option

## 0.5.9

### Patch Changes

- 8c2be87: Fix multi line bindings from Morpho to Builder
...
- 7a099d2: [Builder,Morpho] Feature: add Slots support.
...
- 56c6347: [Morpho] Feature: add `nativeConditionals` and `nativeLoops` for `legacy` mode
...
- dad8e2f: Fix: do not consolidate class and className for Morpho components
...
- c29e3ca: fix index in for loops between morpho<->builder
...
- 7d8f4ff: Correct support bindings netween Builder and Morpho, including useStore() hook
...
- 9abf0ac: Feat: `trackBy` for angular (can be used when the child used inside <For> has a `key` attribute in morpho)
...
- 413cdc2: fix: fix ref issue when transforming morpho code into solid.js code
...
- c8b7883: Fix: parse slots into `MorphoNode` `slots` property.
