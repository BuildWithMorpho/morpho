...
const getCssFromNode = (node: MorphoNode) => {
...
export const components: CompileAwayComponentsMap = {
  ...
  Embed(node: MorphoNode, context, components) {
    return wrapOutput(
      node,
      createMorphoNode({
  ...
  BuilderAccordion(node: MorphoNode, context, components) {
    ...
    const children: MorphoNode[] = accordionItems.map((accordionItem) => {
      ...
      const detailChildren: MorphoNode[] = accordionItem.detail.map((element) =>
        builderElementToMorphoNode(element, {
          includeBuilderExtras: true,
          preserveTextBlocks: true,
        }),
  ...
  CoreSection(node: MorphoNode, context, components) {
  ...
  Columns(node: MorphoNode, context, components) {
  ...
  Image(node: MorphoNode, context, components) {
  ...
  Video(node: MorphoNode, context, components) {
    let aspectRatio = node.bindings.aspectRatio?.code
      ? parseFloat(node.bindings.aspectRatio.code as string)
      : null;
    if (typeof aspectRatio === 'number' && isNaN(aspectRatio)) {
    ...
    const videoContainerNodes: MorphoNode[] = [];
    ...
    const videoContainer = createMorphoNode({
      name: 'div',
...
export const compileAwayBuilderComponentsFromTree = (
  tree: MorphoNode | MorphoComponent,
  components: CompileAwayComponentsMap,
) => {
