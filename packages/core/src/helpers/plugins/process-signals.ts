...
import { getSignalMorphoImportForTarget, mapSignalType } from '../signals';
...
export const getSignalTypePlugin =
  ({ target }: { target: Target }): MorphoPlugin =>
  () => ({
    json: {
      pre: (json) => {
        ...
        if (json.signals?.signalTypeImportName) {
          ...
          const signalMappedImport = getSignalMorphoImportForTarget(target);
