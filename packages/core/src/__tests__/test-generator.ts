...
import { MorphoComponent, Plugin, createTypescriptProject, parseSvelte } from '..';
...
const basicMorpho = getRawFile('./data/basic-custom-morpho-package.raw.tsx');
...
const metaDataPlugin: Plugin = () => ({
  code: {
    pre: (code: string, json: MorphoComponent) => {
...
export const runTestsForJsx = () => {
  test('Remove Internal morpho package', async () => {
    const component = parseJsx((await basicMorpho).code, {
...
export const runTestsForTarget = <X extends BaseTranspilerOptions>({
  ...
  type ParserConfig = {
    name: 'jsx' | 'svelte';
    parser: (args: { code: string; filePath: string }) => Promise<MorphoComponent>;
    testsArray?: Tests[];
  };
