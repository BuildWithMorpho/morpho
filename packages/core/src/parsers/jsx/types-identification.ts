...
import { mapImportDeclarationToMorphoImport } from './imports';
...
export const getSignalMorphoImportForTarget = (target: Target) => {
  const signalType = getSignalMappingForTarget(target);
  if (!signalType) {
    return undefined;
  }
  return mapImportDeclarationToMorphoImport(signalType.importDeclaration);
};

const getSignalSymbol = (project: Project) => {
  const symbolExport = project.createSourceFile(
    'homepage3.lite.tsx',
    `import { Signal } from '@builder.io/morpho';`,
  );
    ...
    ImportSpecifier(path) {
      if (types.isIdentifier(path.node.imported) && path.node.imported.name === 'Signal') {
        if (
          ...
          path.parentPath.node.source.value === '@builder.io/morpho'
        ) {
          /**
           * in case the import is aliased, we need to use the local name,
           * e.g. `import { Signal as MySignal } from '@builder.io/morpho'`
           */
