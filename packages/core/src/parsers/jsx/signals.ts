...
const MORPHO_IMPORT_PATHS = [
  // actual production path
  '/node_modules/@builder.io/morpho/',
  // possible path if symlinking morpho locally
  '/morpho/packages/core/',
];
...
export const findSignals = ({ filePath, project }: { project: Project; filePath: string }) => {
  ...
  const checkIsSignalSymbol = (type: Type<ts.Type>) => {
    ...
    if (MORPHO_IMPORT_PATHS.some((path) => parent.getName().includes(path))) {
      return true;
    }
