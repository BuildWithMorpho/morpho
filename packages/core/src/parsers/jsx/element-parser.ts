...
import type { ParseMorphoOptions } from './types';
const { types } = babel;
...
export const jsxElementToJson = (
  ...
  // const properties: MorphoNode['properties'] = {}
  ...
  const { bindings, properties, slots, blocksSlots } = node.openingElement.attributes.reduce<{
    ...
    slots: {} & MorphoNode['slots'];
    blocksSlots: {} & MorphoNode['blocksSlots'];
  }>(
    (memo, item) => {
      if (types.isJSXAttribute(item)) {
        ...
        if (types.isStringLiteral(expression)) {
          ...
          // <Foo myProp={<MoreMorphoNode><div /></MoreMorphoNode>} />
          ...
          if (
            ...
            /**
             * Find any deeply nested JSX Elements, convert them to Morpho nodes
             * then store them in "replacements" to later do a string substitution
             * to swap out the stringified JSX with stringified Morpho nodes.
             *
            ...
            const replacements: { start: number; end: number; node: MorphoNode }[] = [];
            ...
            if (replacements.length > 0) {
              // Replace stringified JSX (e.g. <Foo></Foo>) with stringified Morpho JSON
