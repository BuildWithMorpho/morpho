...
import type { BaseHook, StateValue } from '../../../types/morpho-component';
...
function addPropertiesAndStateToNode(json: SveltosisComponent, node: MorphoNode) {
...
function postProcessChildren(json: SveltosisComponent, children: MorphoNode[]) {
  for (const node of children) {
    addPropertiesAndStateToNode(json, node);
    processBindings(json, node);

    let children: MorphoNode[] = [];

    if (node.children?.length > 0) {
      children = node.children;
    }

    const metaValues = (Object.values(node.meta) || []) as Array<MorphoNode | MorphoNode['meta']>;

    if (metaValues.length > 0) {
      const metaChildren = metaValues.filter((item) => {
        return item?.['@type'] === '@builder.io/morpho/node';
      }) as MorphoNode[];
