...
import { Binding, BuilderLocalizedValue, MorphoNode } from '../../types/morpho-node';
  ...
  'Shopify:For': (block, options) => {
    return createMorphoNode({
      ...
      children: (block.children || []).map((child) =>
        builderElementToMorphoNode(updateBindings(child, 'state.$index', 'index'), options),
      ),
    });
  },
  Text: (block, options) => {
    ...
    const styleString = getStyleStringFromBlock(block, options);
    const actionBindings = getActionBindingsFromBlock(block, options);
    const localizedValues: MorphoNode['localizedValues'] = {};
    ...
    // Builder uses {{}} for bindings, but Morpho expects {} so we need to convert
...
export const builderElementToMorphoNode = (
  ...
  if (
    ...
    const textProperties = builderElementToMorphoNode(firstChild, options);
...
export const getMetaFromBlock = (block: BuilderElement, options: BuilderToMorphoOptions) => {
...
const getHooks = (content: BuilderContent) => {
  ...
  try {
    ...
    export default function TemporaryComponent() {
      ${
        // Morpho parser looks for useStore to be a variable assignment,
...
// TODO: maybe this should be part of the builder -> Morpho part
...
const builderContentPartToMorphoComponent = (
