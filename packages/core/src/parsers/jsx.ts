...
import { MorphoComponent, MorphoImport, MorphoExport } from '../types/morpho-component';
...
const componentFunctionToJson = (
  ...
  const hooks: MorphoComponent['hooks'] = {};
  let state: MorphoComponent['state'] = {};
  const accessedContext: MorphoComponent['context']['get'] = {};
  const setContext: MorphoComponent['context']['set'] = {};
  const refs: MorphoComponent['refs'] = {};
 ...
 *   setText(...) -> state.text = ...
 */
function mapReactIdentifiers(json: MorphoComponent) {
 ...
 */
function extractContextComponents(json: MorphoComponent) {
  traverse(json).forEach(function (item) {
    if (isMorphoNode(item)) {
...
export function parseJsx(
  ...
  const useOptions: ParseMorphoOptions = {
    format: 'react',
    ...options,
  };
