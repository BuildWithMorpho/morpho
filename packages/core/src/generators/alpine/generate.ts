...
import { ForNode, MorphoNode, checkIsForNode } from '@/types/morpho-node';
...
export const checkIsComponentNode = (node: MorphoNode): boolean =>
  node.name === '@builder.io/morpho/component';
...
const replaceInputRefs = curry((json: MorphoComponent, str: string) => {
...
const getStateObjectString = (json: MorphoComponent) =>
  flow(
...
const mappers: {
  [key: string]: (json: MorphoNode, options: ToAlpineOptions) => string;
} = {
  For: (json, options) =>
    !(
      checkIsForNode(json) &&
...
// TODO: spread support
const blockToAlpine = (json: MorphoNode | ForNode, options: ToAlpineOptions = {}): string => {
