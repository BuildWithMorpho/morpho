...
const getForNames = (json: MorphoComponent) => {
  const names: string[] = [];
  traverse(json).forEach(function (item) {
    if (isMorphoNode(item)) {
      if (item.name === 'For') {
...
const mappers: {
  [key: string]: (json: MorphoNode, options: InternalToHtmlOptions) => string;
} = {
...
const getId = (json: MorphoNode, options: InternalToHtmlOptions) => {
...
// TODO: spread support
const blockToHtml = (json: MorphoNode, options: InternalToHtmlOptions) => {
