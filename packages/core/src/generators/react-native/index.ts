...
import { MorphoNode, MorphoPlugin } from '../..';
...
export const collectReactNativeStyles = (
  ...
  const getStyleSheetName = (item: MorphoNode) => {
 ...
 */
const PROCESS_REACT_NATIVE_PLUGIN: MorphoPlugin = () => ({
  json: {
 ...
 */
const REMOVE_REACT_NATIVE_CLASSES_PLUGIN: MorphoPlugin = () => ({
  json: {
    pre: (json: MorphoComponent) => {
 ...
 */
const TWRNC_STYLES_PLUGIN: MorphoPlugin = () => ({
  json: {
 ...
 */
const NATIVE_WIND_STYLES_PLUGIN: MorphoPlugin = () => ({
  json: {
    post: (json: MorphoComponent) => {
      traverse(json).forEach(function (node) {
        if (isMorphoNode(node)) {
