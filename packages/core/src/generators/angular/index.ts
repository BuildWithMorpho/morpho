...
import { Binding, MorphoNode, checkIsForNode } from '@/types/morpho-node';
...
const mappers: {
  [key: string]: (root: MorphoComponent, json: MorphoNode, options: ToAngularOptions) => string;
} = {
...
const preprocessCssAsJson = (json: MorphoComponent) => {
...
const stringifyBinding =
  (node: MorphoNode, options: ToAngularOptions, blockOptions: AngularBlockOptions) =>
...
const handleNgOutletBindings = (node: MorphoNode, options: ToAngularOptions) => {
  let allProps = '';
...
export const blockToAngular = ({
  ...
  if (checkIsForNode(json)) {
    ...
    str += `</ng-container>`;
    // else condition
    if (isMorphoNode(json.meta?.else)) {
      str += `<ng-container *ngIf="!(${condition})">`;
...
const handleProperties = (json: MorphoComponent, item: MorphoNode, index: number) => {
...
const transformState = (json: MorphoComponent) => {
...
export const componentToAngular: TranspilerGenerator<ToAngularOptions> =
  ...
  ({ component: _component }) => {
    ...
    const getPropsDefinition = ({ json }: { json: MorphoComponent }) => {
