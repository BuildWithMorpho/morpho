...
import { BaseNode, ForNode, MorphoNode } from '../types/morpho-node';
import {
  runPostCodePlugins,
  runPostJsonPlugins,
  runPreCodePlugins,
...
${json.children.map((item) => blockToSvelte({ json: item, options, parentComponent })).join('\n')}
  ...
  {:else}
  ${blockToSvelte({
    json: json.meta.else as MorphoNode,
    options,
    parentComponent,
  })}
  `
...
const getContextCode = (json: MorphoComponent) => {
...
const setContextCode = (json: MorphoComponent) => {
...
const BINDINGS_MAPPER = {
  innerHTML: (json: MorphoNode, options: ToSvelteOptions) =>
...
const getTagName = ({
  ...
  json: MorphoNode;
  parentComponent: MorphoComponent;
}) => {
...
type BlockToSvelte<T extends BaseNode = MorphoNode> = (props: {
  json: T;
...
export const blockToSvelte: BlockToSvelte = ({ json, options, parentComponent }) => {
  ...
  if ((json.bindings.style?.code || json.properties.style) && !isComponent) {
    ...
    str += `use:morpho_styling={${useValue}}`;
