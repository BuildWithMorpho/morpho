...
import { Binding, ForNode, MorphoNode } from '@/types/morpho-node';
...
const componentMappers: {
  [key: string]: (node: MorphoNode, options: ToBuilderOptions) => BuilderElement;
} = {
  ...
  For(_node, options) {
    ...
    const replaceIndex = (node: MorphoNode) => {
      traverse(node).forEach(function (thing) {
        if (!isMorphoNode(thing)) return;
  ...
  Show(node, options) {
    const elseCase = node.meta.else as MorphoNode;
...
const processLocalizedValues = (element: BuilderElement, node: MorphoNode) => {
    ...
    const element = el(
      {
        ...
        component: {
          name: 'Text',
          options: {
            // Morpho uses {} for bindings, but Builder expects {{}} so we need to convert
