...
export function renderJSXNodes(
  ...
  return function (this: SrcBuilder) {
    ...
    children.forEach((child) => {
      ...
      if (isTextNode(child)) {
        ...
        if (typeof directive == 'function') {
          const blockFn = morphoNodeToRenderBlock(child.children);
          const meta = child.meta;
          Object.keys(meta).forEach((key) => {
            const value = meta[key];
            if (isMorphoNode(value)) {
              (blockFn as any)[key] = morphoNodeToRenderBlock([value]);
            }
          });
    ...
    function morphoNodeToRenderBlock(children: MorphoNode[]) {
      return () => {
...
function isEmptyTextNode(child: MorphoNode) {
  return child.properties._text?.trim() == '';
}

function isTextNode(child: MorphoNode) {
...
function isSlotProjection(child: MorphoNode) {
...
function isInlinedDirective(node: MorphoNode) {
  return (isMorphoNode(node) && node.name == 'Show') || node.name == 'For';
}
