...
export function getLexicalScopeVars(component: MorphoComponent) {
  return [
    'props',
    'state',
    ...Object.keys(component.refs),
    ...Object.keys(component.context.get),
    ...Object.keys(component.state).filter((key) => {
      const stateType = component.state[key]?.type;
...
export function getStateMethodsAndGetters(state: MorphoComponent['state']): MethodMap {
