...
import { getOnUpdateHookName, processBinding, renameMorphoComponentsToKebabCase } from './helpers';
 ...
 */
const onUpdatePlugin: MorphoPlugin = (options) => ({
  json: {
    post: (component) => {
  ...
  ({ component: _component, path }) => {
    ...
    const template =
      options.casing === 'kebab'
        ? renameMorphoComponentsToKebabCase(templateStrBody)
        : templateStrBody;
