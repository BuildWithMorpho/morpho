...
const mappers: {
  [key: string]: (json: MorphoNode, options: ToSwiftOptions) => string;
} = {
...
const blockToSwift = (json: MorphoNode, options: ToSwiftOptions): string => {
...
function getActionsString(json: MorphoNode, options: ToSwiftOptions): string {
  let str = '';
...
function getStyleString(node: MorphoNode, options: ToSwiftOptions): string {
  const style = getStyles(node);
  let str = '';
  for (const key in style) {
    let useKey = key;
    const rawValue = style[key as keyof MorphoStyles]!;
...
function getJsSource(json: MorphoComponent, options: ToSwiftOptions) {
...
function mapDataForSwiftCompatability(json: MorphoComponent) {
  let inputIndex = 0;
...
function getInputBindings(json: MorphoComponent, options: ToSwiftOptions) {
  let str = '';
