...
const renameMorphoComponentsToKebabCase = (str: string) =>
  str.replace(/<\/?\w+/g, (match) => match.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase());

function getContextNames(json: MorphoComponent) {
...
type BlockRenderer = (json: MorphoNode, options: ToVueOptions, scope?: Scope) => string;
...
const NODE_MAPPERS: {
  ...
  For(_json, options) {
    ...
    const firstChild = json.children.filter(filterEmptyTextNodes)[0] as MorphoNode | undefined;
  ...
  Show(json, options, scope) {
    ...
    switch (options.vueVersion) {
      ...
      case 2:
        ...
        const firstChild = children[0] as MorphoNode | undefined;
        ...
        if (allShowChildrenWithoutElse && isMorphoNode(elseBlock)) {
          /**
          ...
          firstChild &&
          isMorphoNode(elseBlock) &&
          hasShowChild &&
          isMorphoNode(childElseBlock)
        ) {
          /**
          ...
          const elseString = isMorphoNode(elseBlock)
...
function processDynamicComponents(json: MorphoComponent, _options: ToVueOptions) {
...
function processForKeys(json: MorphoComponent, _options: ToVueOptions) {
  traverse(json).forEach((node) => {
    if (isMorphoNode(node)) {
      if (node.name === 'For') {
...
function getContextProvideString(component: MorphoComponent, options: ToVueOptions) {
  let str = '{';
...
function appendValueToRefs(input: string, component: MorphoComponent, options: ToVueOptions) {
...
const componentToVue: TranspilerGenerator<ToVueOptions> =
  ...
  ({ component, path }) => {
    ...
    const template = pipe(
      ...
      renameMorphoComponentsToKebabCase,
    );
