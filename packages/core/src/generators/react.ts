...
const wrapInFragment = (json: MorphoComponent | MorphoNode) =>
  json.children.length !== 1;

const NODE_MAPPERS: {
  [key: string]: (json: MorphoNode, options: ToReactOptions) => string;
} = {
...
export const blockToReact = (json: MorphoNode, options: ToReactOptions) => {
