...
const NODE_MAPPERS: {
  [key: string]: (json: MorphoNode, options: ToReactOptions, parentSlots?: any[]) => string;
} = {
  Slot(json, options, parentSlots) {
    if (!json.bindings.name) {
      // TODO: update MorphoNode for simple code
...
export const blockToReact = (json: MorphoNode, options: ToReactOptions, parentSlots?: any[]) => {
...
const getRefsString = (json: MorphoComponent, refs: string[], options: ToReactOptions) => {
...
const getUseStateCode = (json: MorphoComponent, options: ToReactOptions) => {
  let str = '';
...
const updateStateSetters = (json: MorphoComponent, options: ToReactOptions) => {
  if (options.stateType !== 'useState') {
    return;
  }
...
function getContextString(component: MorphoComponent, options: ToReactOptions) {
  let str = '';
...
const getInitCode = (json: MorphoComponent, options: ToReactOptions): string => {
