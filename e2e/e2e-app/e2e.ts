// Morpho E2E orchestration script
...
// Update this array when Morpho adds new targets
...
async function detectFailures(allResults: any[]) {
  ...
  // allowed so that the case and progress can be merged, even where Morpho
...
async function main() {
  ...
  for (const c of cases) {
    ...
    // https://github.com/BuilderIO/morpho/issues/494
    ...
    // Morpho all targets - with a workaround to tolerate failure, until:
    // https://github.com/BuilderIO/morpho/issues/510
    await yarn('workspace', '@builder.io/e2e-app', 'run', 'run-morpho-separately');
